  ...
  submit: function(e){
    ...
    app.util.request({
      ...
      data: {
        ...
        area: (that.data.objectAreas == undefined || that.data.objectAreas.length == 0) ? '' : that.data.objectAreas[that.data.areasIndex].name
      },
      method: "POST",
      success: function(res){
        if(res.data.errno == 1){
    ...
    */
    ...
    if(that.data.realname == '' || that.data.realname == undefined){
      wx.showModal({
        title: '温馨提示',
        content: '姓名不能为空',
        showCancel: false
    ...
    if(that.data.staticIndex == 0 && (that.data.id_card == '' || that.data.id_card == undefined)){
      wx.showModal({
        title: '温馨提示',
        content: '身份证号不能为空',
        showCancel: false
    ...
    app.util.request({
      ...
      data: {
        ...
        address: that.data.objectProvinces[that.data.provincesIndex].name + ' ' + that.data.objectCitys[that.data.citysIndex].name + ' ' + that.data.objectDistricts[that.data.districtsIndex].name + ((that.data.objectAreas == undefined || that.data.objectAreas.length == 0) ? '' : ' ' + that.data.objectAreas[that.data.areasIndex].name)
      },
      method: "POST",
      success: function(res){
        if(res.data.errno == 0){
          wx.showModal({
            title: '温馨提示',
            content: '申请成功，请等待审核，如需加速处理请联系客服',
            showCancel: false,
            success: function(res){
              that.onShow();
            }
