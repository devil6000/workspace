...
* 获取地理位置信息
*/
util.getLocation = function (option) {
  ...
  wx.getLocation({
    ...
    success(res) {
      ...
      console.log(timestamp + option.cachetime * 1000 ) 
      var cachedata = { 'data': { 'lat': latitude, 'lng': longitude }, 'expire': timestamp + option.cachetime * 1000 };
      wx.setStorageSync('getLocation', cachedata);
      if (option.success && typeof option.success == 'function') {
        option.success(cachedata);
        return;
      }
...
/**
	构造巨吉物流地址, 
	@params action 巨吉物流系统中的controller, action, do，格式为 'wxapp/home/navs'  ,is_debug是否调试模式url
	@params querystring 格式为 {参数名1 : 值1, 参数名2 : 值2}
*/
util.url = function (action, querystring) {
	var app = getApp();

  var is_debug = action.is_debug && action.is_debug!=1 ?0:1;
  if (is_debug && app.siteInfo.debug){
    var url = app.siteInfo.debug_siteroot + '?i=' + app.siteInfo.uniacid + '&t=' + app.siteInfo.multiid + '&v=' + app.siteInfo.version + '&from=wxapp&';
  }else{
...
/**
	二次封装微信wx.request函数、增加交互体全、配置缓存、以及配合巨吉物流格式化返回数据
...
util.getUserInfo = function (cb) {
		...
		wx.login({
          ...
          is_debug:0,
					...
					success: function (session) {
                      ...
                      typeof cb == "function" && cb( userInfo.memberInfo);
                      return;
										}
									});
								},
                ...
                //   if (typeof (res.data) != "undefined" &&!res.data.errno) {
                //     userInfo.memberInfo = res.data.data;
                //     wx.setStorageSync('userInfo', userInfo);
                //   }
                //   typeof cb == "function" && cb(userInfo.memberInfo);
  ...
  if (app.memberInfo) {
		wx.checkSession({
			success: function(){
        typeof cb == "function" && cb(app.memberInfo);
			},
			fail: function(){
				app.sessionid = '';
				console.log('relogin');
...
//根据业务逻辑修改底部导航(disonLiu)
util.footer = function ($this) {
	let app = getApp();
	let that = $this;
  var userType = wx.getStorageSync('userType')
  if (userType =='worker'){
    var tabBar = app.workerTabBar;
  } else if (userType == 'issuer'){
    var tabBar = app.issuerTabBar;
  }else{
    return;
  }
