...
var webSocket = {
  ...
  connectSocket: function (options) {
    try{
      if (options){
        wx.setStorageSync('socket_options', options)
      }
      options = options ? options : wx.getStorageSync('socket_options');
    }catch(res){
    ...
    socketMsgQueue = []
    wx.connectSocket({
      url: options.url ? options.url: 'wss://www.jujiwuliu.com/jujiwuliu',
  ...
  heartBeatAccessory: function (msg=''){
    console.log(msg)
    //心跳附加方法
  },
  // 开始心跳
